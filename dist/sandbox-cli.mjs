...
var WORKSPACE_ROOT = "/workspace/repo";
function resolveWorkspacePath(relativePath = ".") {
  if (relativePath.includes("\0")) {
    throw new Error("Workspace path contains a null byte.");
  }
  if (path4.posix.isAbsolute(relativePath)) {
...
// src/preview-config.ts
var PREVIEW_CONFIG_PATH = ".github/claude-triage.yml";
var MAX_VALIDATION_COMMANDS = 20;
var MAX_COMMAND_CHARS = 1e4;
...
function parsePreviewConfig(yamlText) {
  const document = (0, import_yaml.parse)(yamlText);
  if (!isRecord(document) || document.preview === void 0) {
    return void 0;
  }
...
function describePreviewValidation(outcome) {
  switch (outcome.status) {
    ...
    case "failed":
      return `Deterministic preview validation failed: ${JSON.stringify(outcome.command)} exited with code ${outcome.exitCode}.`;
  }
}
...
async function runPreviewValidation(client, sandboxId, repositoryDirectory, nodeBinPath, previewDirectory, timeoutMs) {
  if (!previewDirectory) {
    return { status: "skipped", reason: "no preview directory is configured" };
  }
  const configText = await readOptionalFile(path5.join(repositoryDirectory, PREVIEW_CONFIG_PATH));
  ...
  if (config.directory !== previewDirectory) {
    throw new Error(
      `${PREVIEW_CONFIG_PATH} configures preview directory ${JSON.stringify(config.directory)} but the action received ${JSON.stringify(previewDirectory)}.`
    );
  }
...
`);
  ...
  if (command === "hydrate-worktree") {
    ...
    const repositoryDirectory = path5.resolve(requiredArgument(args[1], "repository directory"));
...
`);
  ...
  if (command === "validate-preview") {
    ...
    const outputPath = path5.resolve(requiredArgument(args[5], "validation output path"));
...
`);
  ...
  if (command === "export-patch") {
    const sandboxId = requiredArgument(args[0], "sandbox ID");
    const outputPath = path5.resolve(requiredArgument(args[1], "patch output path"));
  ...
  if (command === "upload-issue-context") {
    const sandboxId = requiredArgument(args[0], "sandbox ID");
    const inputPath = path5.resolve(requiredArgument(args[1], "issue context path"));
    await client.writeFile(sandboxId, "/workspace/issue.json", await readFile3(inputPath, "utf8"));
    return;
  }
  if (command === "upload-triage-context") {
    const sandboxId = requiredArgument(args[0], "sandbox ID");
    const artifactDirectory = path5.resolve(requiredArgument(args[1], "triage artifact directory"));
    const files = [
    ...
    for (const [sourceName, destinationPath] of files) {
      await client.writeFile(
        sandboxId,
        destinationPath,
        await readFile3(path5.join(artifactDirectory, sourceName), "utf8")
      );
    }
    return;
  }
  if (command === "mcp-config") {
    const serverPath = path5.resolve(requiredArgument(args[0], "MCP server path"));
